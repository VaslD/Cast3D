public struct CastMeshNode {
    ...
    public let vertexWeightBoneBuffer: [RawPropertyValue]?
    public let vertexWeightValueBuffer: [Float]?

    public init?(_ node: RawNode) {
        guard node.header.identifier == .mesh else { return nil }
        ...
        var vertexWeightBoneBuffer: [RawPropertyValue]?
        var vertexWeightValueBuffer: [Float]?
        ...
        for property in node.properties {
            ...
            case "wb":
                ...
                vertexWeightBoneBuffer = property.values
            case "wv":
                guard property.header.identifier == .float else { return nil }
                vertexWeightValueBuffer = property.values.compactMap { $0.value as? Float }
            default:
                break
            }
        }
        ...
        self.vertexWeightBoneBuffer = vertexWeightBoneBuffer
        self.vertexWeightValueBuffer = vertexWeightValueBuffer
    }
}
